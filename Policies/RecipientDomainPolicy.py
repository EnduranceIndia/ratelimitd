...
class RecipientDomainPolicy:
    """
    This class provides recipient domain rate limiting
    """
    key = 'recipient'
    prefix = 'RecipientDomainPolicy_'
    quota = {}

    def __init__(self, parsed_config):
        self.parsed_config = parsed_config
        self.Enforce = parsed_config.getboolean('RecipientDomainPolicy', 'Enforce')
        self.RejectMessage = parsed_config.get('RecipientDomainPolicy', 'RejectMessage')
        self.ProfileLookupObj = ProfileLookup.create_profile_lookup('RecipientDomainPolicy', parsed_config)
        self.ProfileCacheTTL = parsed_config.getint('RecipientDomainPolicy', 'ProfileCacheTime')
        for i in parsed_config.items('RecipientDomainPolicy-Profiles'):
            limits = i[1].split(',')
            profile = i[0].lower()
            RecipientDomainPolicy.quota[profile] = (int(limits[0]), int(limits[1]))
    ...
    def check_quota(self, message, redis_pipe):
        ...
        try:
            ...
            RedisConn.LUA_CALL_CHECK_LIMIT(keys=[RecipientDomainPolicy.prefix + self.value],
    ...
    def log_quota(self, message, accept, redis_val=None):
        if accept:
            ...
            Logger.log('RecipientDomainPolicy RecipientDomain: %s Quota: Exceeded Profile: %s Action: reject' % (
